...
public class BoardController {  
  ...
  public void delete(Scanner keyScan) {      
    try {
      this.list();
      System.out.print("삭제하고 싶은 게시판 번호를 입력하세요. ");
  ...
  public void list() {
    try {
      ...
      System.out.println("저장된 게시판 목록입니다.");
      for (Board board : boards) {

        System.out.printf("%d번 %s, %s, %s, %s, %d\n", board.getBno(),
            board.getTitle(), board.getWriter(), board.getContent(),        
            board.getCreatedDate(), board.getViews());
  ...
  public void update(Scanner keyScan) {    
    ...
    try { 
      this.list();
      System.out.print("변경하고 싶은 게시판 번호를 입력하세요. ");
      int no = Integer.parseInt(keyScan.nextLine());      
