...
public class StartUpHandlerMapping {
  ApplicationContext appContext;
  HashMap<String, RequestHandler> handlerMap = new HashMap<>();
  
  public StartUpHandlerMapping(ApplicationContext appContext) {
    ...
    Method[] methods = null;
    StartUp anno = null;
    StartUp classAnno = null;
    String baseName = null;
    
    for (Object controller : controllers) {
      // 클래스에 @RequestMapping이 붙어있다면, 기본 이름으로 저장한다.
      classAnno = controller.getClass().getAnnotation(StartUp.class);
      if (classAnno != null) {
      ...
      for (Method m : methods) {
        anno = m.getAnnotation(StartUp.class);
        if (anno == null) {
          continue;
        }
